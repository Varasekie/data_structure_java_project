...
public class SinglyList<T> {
    public DoubleNode<T> head;                         // 头指针，指向单链表的头结点
    //注意，head不能是其他权限，因为7.2.2节，图的邻接表中，删除顶点要遍历。2014年8月3日

    //（1）构造方法
    public SinglyList()                          //构造方法，构造空单链表
    {
        this.head = new DoubleNode<>();               //创建头结点，data和next值均为null
    }

    //构造单链表，尾插入values数组元素，忽略其中空对象。单链表元素次序与数组元素次序相同。O(n)
    public SinglyList(T[] values) {
        this();                                  //创建空单链表，只有头结点
        DoubleNode<T> rear = this.head;                //rear指向单链表最后一个结点
        for (int i = 0; i < values.length; i++)       //若values.length==0，则构造空链表
        {
            if (values[i] != null) {
                rear.next = new DoubleNode<T>(values[i], null);  //尾插入，创建结点链入rear结点之后
...
//                str += ",";
    ...
    public DoubleNode<T> insert(int i, T x) {
        ...
        for (int j = 0; front.next != null && j < i; j++)  //寻找第i-1个或最后一个结点（front指向）
            front = front.next;
        front.next = new DoubleNode<T>(x, front.next); //在front之后插入值为x结点，包括头插入、中间/尾插入
        return front.next;
    }

    public DoubleNode<T> insert(T x)                   //单链表尾插入x，O(n)，重载
    {
...
//        throw new IndexOutOfBoundsException(i+"");       //抛出序号越界异常
    ...
    //8.3 散列集合用，
    public DoubleNode<T> search(T key)                 //顺序查找并返回首个与key相等元素结点，若查找不成功，则返回null
    {
...
//          System.out.print(p.data.toString()+"？");
    ...
    public T remove(T key) {
        ...
        for (DoubleNode<T> front = this.head, p = front.next; p != null; front = p, p = p.next) {
    ...
    public void copy(SinglyList<T> list)         // 复制list所有元素到this，放弃this原结点。O(n)
    {
        ...
        for (DoubleNode<T> p = list.head.next; p != null; p = p.next) //p遍历list单链表
        {
            rear.next = new DoubleNode<T>(p.data, null);//复制结点尾插入到this单链表；对象引用，没有复制
    ...
    public boolean equals(Object obj)            //比较this与obj引用的两条单链表是否相等，覆盖Object类的equals(obj)方法
    {
        ...
        if (obj instanceof SinglyList<?>) {
            DoubleNode<T> p = this.head.next;
            DoubleNode<T> q = ((SinglyList<T>) obj).head.next;
    ...
    {
        DoubleNode<T> current = SinglyList.this.head;    //当前结点，初值为外部类单链表头结点
        DoubleNode<T> front = null;                      //当前结点的前驱结点
