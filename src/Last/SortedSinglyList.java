...
public class SortedSinglyList <T extends Comparable<? super T>> extends SinglyList<T>{
    ...
    public DoubleNode<T> insert(T x)
    {
        ...
        DoubleNode<T> front=this.head,  p=front.next;            //front指向p的前驱结点
        while(p!=null && (this.asc ? x.compareTo(p.data)>=0 : x.compareTo(p.data)<=0))
        {
            front = p;
            p = p.next;
        }
        front.next = new DoubleNode<T>(x, p);                    //在front之后、p之前插入值为x结点
...
//        System.out.print(this.getClass().getName()+".search("+key+")，");
    ...
    {
        DoubleNode<T> front=this.head, p=front.next;   //front是p的前驱结点
    ...
    {
        DoubleNode<T> front = head.next, p=front.next;
...
//    }
    ...
    {
        for(DoubleNode<T> p=list.head.next;  p!=null;  p=p.next)//直接插入排序，每趟插入1个元素
    ...
    public SortedSinglyList(SinglyList<T> list, boolean asc)
    {
        ...
        for(DoubleNode<T> first=this.head.next; first.next!=null; first=first.next)
        {                                        //first指向待排序单链表首个结点
            DoubleNode<T> min=first;                   //min指向最小值结点
            for(DoubleNode<T> p=min.next;   p!=null;   p=p.next) //遍历单链表，寻找最小值结点
...
//            System.out.println(this.toString());
    ...
    public void merge(SortedSinglyList<T> list)
    {
        ...
        {
            for(DoubleNode<T> p=list.head.next; p!=null; p=p.next) //直接插入排序
                this.insert(p.data);             //排序单链表按值插入
            return;
        }

        DoubleNode<T> front=this.head,  p=front.next;  //p遍历this单链表，front是p的前驱
        DoubleNode<T> q=list.head.next;                //q遍历list单链表
...
/*    public SortedSinglyList<T> mergeWith(SortedSinglyList<T> list)
    ...
    public SortedSinglyList<T> mergeWith(SortedSinglyList<T> list)
    {
        ...
        while(p!=null || q!=null)                //p、q分别遍历this、list单链表
        {
            ...
            {
                rear.next = new DoubleNode<T>(q.data,null);
                rear = rear.next;
                q = q.next;
            }
        }
    ...
    public static void main(String[] args) {
        System.out.println("归并后的排序单链表");
        System.out.println("ABCDHGM");
    }
}
