...
public class SortedSinglyList<T extends Comparable<? super T>> extends SinglyList<T> {
    ...
    public Node<T> insert(T x) {
        ...
        Node p;
        for(p = front.next; p != null && x.compareTo((T)p.data) > 0; p = p.next) {
            front = p;
        }
    ...
    public Node<T> search(T key) {
        for(Node p = this.head.next; p != null && key.compareTo((T)p.data) >= 0; p = p.next) {
            if (key.compareTo((T)p.data) == 0) {
                return p;
            }
        }
    ...
    public Node<T> insertDifferent(T x) {
        ...
        return p != null && x.compareTo((T)p.data) == 0 ? p : (front.next = new Node(x, p));
    }
    ...
    public T remove(T key) {
        ...
        Node p;
        for(p = front.next; p != null && key.compareTo((T)p.data) > 0; p = p.next) {
            front = p;
        }

        if (p != null && key.compareTo((T)p.data) == 0) {
            front.next = p.next;
            return (T)(Comparable)p.data;
        } else {
            return null;
        }
    }
